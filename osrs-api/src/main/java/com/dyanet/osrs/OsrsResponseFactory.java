...
/*    private void processArray(Node item) {
    ...
    private OsrsResponse createOsrsResponse(String model) throws OsrsException {
        ...
        try {
          Class<?> clazz = Class.forName("com.dyanet.osrs.resp." + model + "Response");
          Class<?> clayy;
          if (isList) {
              clayy = Class.forName("java.util.List");
          } else {
              clayy = Class.forName("com.dyanet.osrs.model." + model);
          }
          Method method = clazz.getDeclaredMethod("set"+model, clayy);
          OsrsResponse r = (OsrsResponse)clazz.getDeclaredConstructor().newInstance();
            if (r != null) {
                Object m = null;
                if (model.endsWith("s")) {
                    m = new ArrayList<Object>();
                } else {
                    m = clayy.getDeclaredConstructor().newInstance(); // the model
                }
